...
export default function SignUp() {
    ...
    const handleSignUp = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (doesUsernameExistResult && doesUsernameExistResult.length === 0) {
            try {
                const createdUserResult = await firebase.firebaseInit.firebase_.auth().createUserWithEmailAndPassword(emailAddress, password);
                if(createdUserResult.user)
                    await createdUserResult.user.updateProfile({
                        displayName: username
                    });
                if(createdUserResult.user)
                    await firebase.firebaseInit.firebase_.firestore().collection('users').add({
                        userId: createdUserResult.user.uid,
                        username: username.toLowerCase(),
                        displayName: username.toLocaleLowerCase(),
                        fullName,
                        emailAddress: emailAddress.toLowerCase(),
                        following: ['fQxKrFoskoaNlKVZ570TwtwAPC22'],
                        followers: [],
